...
(function () {
  ...
  // --- Video heading: char-by-char reveal (same style as video-outro) ---
  if (heading && !isMobile) {
    var headingChars = [];
    // Walk through child nodes to preserve <span> and <br> tags
      ...
      if (node.nodeType === 3) { // text node
        ...
        for (var ci = 0; ci < text.length; ci++) {
          if (text[ci] === ' ') {
            ...
            cs.style.opacity = '0.15';
            cs.style.display = 'inline';
            heading.appendChild(cs);
            headingChars.push(cs);
          }
        }
      } else if (node.nodeName === 'BR') {
        heading.appendChild(document.createElement('br'));
      } else {
        // Element node (e.g. <span class="heading-italic">)
        var wrapper = node.cloneNode(false);
        var innerText = node.textContent;
        for (var ci = 0; ci < innerText.length; ci++) {
          if (innerText[ci] === ' ') {
            wrapper.appendChild(document.createTextNode(' '));
          } else {
            var cs = document.createElement('span');
            cs.className = 'char';
            cs.textContent = innerText[ci];
            cs.style.opacity = '0.15';
            cs.style.display = 'inline';
