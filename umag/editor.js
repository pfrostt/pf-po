...
(function () {
  ...
  style.textContent =
    ...
    /* Edit mode: make tagline and hero text clickable above overlapping elements */
    'body.editor-active .hero__tagline{position:relative;z-index:100;pointer-events:auto}' +
    'body.editor-active .hero__greeting{position:relative;z-index:100}' +
    'body.editor-active .hero__main{z-index:100}' +
    'body.editor-active .hero-fade-simple,body.editor-active .hero-fade{opacity:1!important;animation:none!important}' +
    /* Edit mode: make all outros/reveals visible and clickable */
    'body.editor-active .about-outro,body.editor-active .clients-outro,body.editor-active .video-outro,body.editor-active .skills-outro__text,body.editor-active .proof-closing{opacity:1!important;position:relative!important;z-index:10;pointer-events:auto!important}' +
    'body.editor-active .char{opacity:1!important}' +
    'body.editor-active .reveal{opacity:1!important;transform:none!important}' +
    'body.editor-active .about-intro{opacity:1!important;position:relative;z-index:10}' +
    'body.editor-active .clients-intro{opacity:1!important;position:relative;z-index:10}' +
    ...
    '#editor-toolbar{position:fixed;z-index:10001;display:none;background:#1e1e2e;border-radius:8px;padding:4px;box-shadow:0 4px 16px rgba(0,0,0,0.4);gap:2px;align-items:center}' +
    '#editor-toolbar.is-visible{display:flex}' +
    '#editor-toolbar button{width:32px;height:32px;border:none;border-radius:6px;background:transparent;color:#cdd6f4;font:600 14px system-ui,sans-serif;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s}' +
    '#editor-toolbar button:hover{background:#313244}' +
    '#editor-toolbar button.is-active{background:#2563eb;color:#fff}' +
    '#editor-toolbar button .tb-i{font-style:italic}' +
    '#editor-toolbar button .tb-u{text-decoration:underline}' +
    '#editor-toolbar .tb-divider{width:1px;height:20px;background:#45475a;margin:0 2px}' +
    '#editor-toolbar select{height:32px;border:none;border-radius:6px;background:#313244;color:#cdd6f4;font:600 12px system-ui,sans-serif;cursor:pointer;padding:0 6px;outline:none}' +
    '#editor-toolbar select:hover{background:#45475a}' +
  ...
  toolbar.innerHTML =
    ...
    '<button data-cmd="italic" title="Italic (Cmd+I)"><span class="tb-i">I</span></button>' +
    '<button data-cmd="underline" title="Underline (Cmd+U)"><span class="tb-u">U</span></button>' +
    '<div class="tb-divider"></div>' +
    '<select id="ed-weight" title="Font weight">' +
  ...
  var weightSelect = document.getElementById('ed-weight');
  ...
  function getSelectionWeight() {
    ...
    var w = window.getComputedStyle(node).fontWeight;
    return w;
  }
  ...
  function updateToolbarState() {
    ...
    var w = getSelectionWeight();
    weightSelect.value = w && weightSelect.querySelector('option[value="' + w + '"]') ? w : '';
  }
  ...
  function positionToolbar() {
    ...
    /* Check if selection is inside an editable config element */
    var node = sel.anchorNode;
    var editable = null;
    while (node) {
    ...
    if (ty < 40) ty = rect.bottom + 8;
    toolbar.style.left = Math.max(4, tx) + 'px';
  ...
  function persistFromSelection() {
    var sel = window.getSelection();
    if (!sel) return;
    var node = sel.anchorNode;
    while (node) {
      if (node.nodeType === 1 && node.hasAttribute('data-config')) {
        var key = node.getAttribute('data-config');
        if (workingConfig.text[key] !== undefined) {
          workingConfig.text[key] = node.innerHTML;
          var dup = document.querySelector('[data-config-title-dup="' + key + '"]');
          if (dup) dup.innerHTML = node.innerHTML;
          persist();
        }
        break;
      }
  ...
  /* Save selection range before dropdown steals focus */
  var savedRange = null;
  var savedConfigEl = null;

  weightSelect.addEventListener('mousedown', function (e) {
    e.stopPropagation();
    var sel = window.getSelection();
    if (sel && sel.rangeCount && !sel.isCollapsed) {
      savedRange = sel.getRangeAt(0).cloneRange();
      /* Walk up to find the data-config parent */
      var node = sel.anchorNode;
      savedConfigEl = null;
      while (node) {
        if (node.nodeType === 1 && node.hasAttribute('data-config')) {
  ...
  weightSelect.addEventListener('change', function () {
    var w = weightSelect.value;
    if (!w || !savedRange) { weightSelect.value = ''; return; }
    ...
    /* Persist to config */
    if (savedConfigEl) {
      var key = savedConfigEl.getAttribute('data-config');
      if (key && workingConfig.text[key] !== undefined) {
        workingConfig.text[key] = savedConfigEl.innerHTML;
        var dup = document.querySelector('[data-config-title-dup="' + key + '"]');
        if (dup) dup.innerHTML = savedConfigEl.innerHTML;
        persist();
      }
    }
