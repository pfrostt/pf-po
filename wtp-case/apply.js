...
/* --- Cinematography Section: scroll-driven panel switching --- */
(function () {
  if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;

  var section = document.getElementById('cinematography');
  if (!section) return;

  var isMobile = window.matchMedia('(max-width: 767px)').matches;

  if (isMobile) {
    // Mobile: restructure into paired visual+text blocks
    var mobileVisuals = section.querySelectorAll('.cine__visual');
    var mobilePanels  = section.querySelectorAll('.cine__text-panel');
    var mobileTitle   = section.querySelector('.cine__title');
    var mobileNav     = section.querySelector('.cine__nav');
    var textCol       = section.querySelector('.cine__text');

    var navLabels = section.querySelectorAll('.cine__nav-item');

    // Clone text panels into their matching visual blocks
    mobileVisuals.forEach(function (visual, i) {
      // Add subsection label
      if (navLabels[i]) {
        var label = document.createElement('h3');
        label.className = 'cine__mobile-label';
        label.textContent = navLabels[i].textContent;
        visual.insertBefore(label, visual.firstChild);
      }
      if (mobilePanels[i]) {
        var clone = mobilePanels[i].cloneNode(true);
        clone.classList.add('is-active');
        clone.classList.add('cine__text-panel--mobile');
        visual.appendChild(clone);
      }
    });

    // Hide the original text column on mobile
    if (textCol) textCol.style.display = 'none';

    return;
  }

  var inner    = section.querySelector('.cine__inner');
  var navItems = section.querySelectorAll('.cine__nav-item');
  var visuals  = section.querySelectorAll('.cine__visual');
  var panels   = section.querySelectorAll('.cine__text-panel');
  var video    = section.querySelector('.cine__visual[data-cine-visual="4"] video');
  var totalPanels = 5;
  ...
  function setActive(index) {
    ...
    if (navItems[index]) navItems[index].classList.add('is-active');
    // Visuals
  ...
  ScrollTrigger.create({
    ...
    onUpdate: function (self) {
      var progress = self.progress;
      var index = Math.min(totalPanels - 1, Math.floor(progress * totalPanels));
      setActive(index);
    }
  });
})();

/* --- Audio Section: dual player with mutual exclusion --- */
(function () {
  var preAudio = document.getElementById('audioPreEdit');
  var postAudio = document.getElementById('audioPostEdit');
  if (!preAudio || !postAudio) return;
  ...
  players.forEach(function (player) {
    var key = player.getAttribute('data-audio');
    var audio = audioMap[key];
    if (!audio) return;

    var playBtn = player.querySelector('.audio-player__play');
    var playIcon = player.querySelector('.audio-player__icon-play');
    var pauseIcon = player.querySelector('.audio-player__icon-pause');
    var waveEl = player.querySelector('.audio-player__wave');
    ...
    function pauseOthers() {
      Object.keys(audioMap).forEach(function (k) {
        if (k !== key) {
          audioMap[k].pause();
          var otherPlayer = document.querySelector('.audio-player[data-audio="' + k + '"]');
          if (otherPlayer) {
            otherPlayer.querySelector('.audio-player__icon-play').style.display = '';
            otherPlayer.querySelector('.audio-player__icon-pause').style.display = 'none';
          }
        }
      });
    }
...
(function () {
  var compare = document.getElementById('gradeCompare');
  var slider = document.getElementById('gradeSlider');
  if (!compare || !slider) return;

  var afterImg = compare.querySelector('.grade__img--after');
...
/* --- Page Index: highlight active section based on scroll --- */
(function () {
  var indexRows = document.querySelectorAll('.wtp-page-index__row[data-index-section]');
  if (!indexRows.length) return;

  var sections = [];
  indexRows.forEach(function (row) {
    var id = row.getAttribute('data-index-section');
    var el = document.getElementById(id);
    if (el) sections.push({ row: row, el: el });
  });

  function updateIndex() {
    var threshold = window.innerHeight * 0.95;
    var activeRow = null;

    for (var i = sections.length - 1; i >= 0; i--) {
      var rect = sections[i].el.getBoundingClientRect();
      if (rect.top <= threshold) {
        activeRow = sections[i].row;
        break;
      }
    }

    indexRows.forEach(function (row) { row.classList.remove('is-active'); });
  ...
  // Click to scroll to section (section top at 10vh)
  indexRows.forEach(function (row) {
...
/* --- Motion Graphics Section: slider with dot navigation --- */
(function () {
  var slider = document.getElementById('motionSlider');
  if (!slider) return;

  var slides = slider.querySelectorAll('.motion__slide');
  var dots = slider.querySelectorAll('.motion__dot');
  var currentIndex = 0;
  ...
  dots.forEach(function (dot) {
    dot.addEventListener('click', function () {
      var index = parseInt(dot.getAttribute('data-motion-dot'), 10);
      goTo(index);
    });
  });
