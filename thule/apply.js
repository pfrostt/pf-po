...
/* --- Box Showcase: scroll-driven unboxing animation --- */
(function () {
  if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;
  ...
  if (isMobile) {
    ...
    if (solOpenBox && solClosedBox) {
      ...
      solMobileTl.to(solClosedBox, {
        x: function () { return solOpenBox.offsetWidth / 2; },
        duration: 1, ease: 'none'
      }, 0);
      solMobileTl.to(solOpenBox, {
    ...
    // --- Desktop: pinned timeline (mirrors Axis video section) ---
    var tl = gsap.timeline({
      scrollTrigger: {
        trigger: section,
        start: 'top top',
    ...
    // 0→0.40: Box scales up from small to full, moves up to make room for text below
    tl.fromTo(boxShowcase,
      { scale: 0.3, rotate: 3, y: 0, borderRadius: '48px' },
      { scale: 1, rotate: 0, y: '-20vh', borderRadius: '16px', duration: 0.40, ease: 'none' },
      0
    );

    // 0.10→0.35: Heading fades out
    if (heading) {
      tl.fromTo(heading, { opacity: 1, y: 0 }, { opacity: 0, y: -30, duration: 0.25, ease: 'none' }, 0.10);
    }
    // 0.15→0.35: Sub fades out
    if (sub) {
      tl.fromTo(sub, { opacity: 1, y: 0 }, { opacity: 0, y: -30, duration: 0.20, ease: 'none' }, 0.15);
    }

    // 0.40→0.70: Unboxing — closed slides right, open slides left
    if (solOpenBox && solClosedBox) {
      tl.to(solClosedBox, {
        x: function () { return solOpenBox.offsetWidth / 2; },
    ...
    // 0.45→0.65: "Now imagine this..." fades in during unboxing
    if (text1) {
      tl.fromTo(text1, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.20, ease: 'none' }, 0.45);
    }

    // 0.72→0.92: "But did you also know..." char-by-char reveal after unboxing completes
    if (videoOutro) {
      var outroStart = 0.72;
      ...
      tl.to(videoOutro, { top: '85vh', duration: 0.20, ease: 'none' }, outroStart);
    }

    // --- Position footer responsively below box ---
    var footer = section.querySelector('.video-section__footer');
    function positionFooter() {
      if (!footer) return;
      // Place footer below the box's final position
      footer.style.top = '65vh';
    }
    positionFooter();
